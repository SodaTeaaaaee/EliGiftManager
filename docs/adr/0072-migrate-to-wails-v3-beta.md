...
版本策略是不钉死版本、跟随最新 beta。跟随最新 beta 是升级时的操作策略；落地形态是 package.json 与 go.mod 的精确版本钉住，升级即一次显式的版本号变更与绑定重生成事件。每次升级是一次显式事件：`wails3` CLI 与 `go.mod` 里的库版本同步升级、重新生成绑定、跑一遍既定冒烟清单（导入、本地图片、对话框、导出与写回、便携模式、缩放、`wails3 build` 产物）；本机保留一个 last-known-good 的 exe 作为回退。stable 发布后自然收敛，升级节奏届时再定。
