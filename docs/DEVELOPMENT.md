...
当前项目仍处于初期开发阶段，因此问题分级不应默认以“历史兼容性最完整”为目标，而应优先服务于当前结构、当前写入路径和从零初始化后的可用性。

默认按以下标准判断问题是否需要立即修复：

- 如果问题在“彻底删除数据库并从零开始使用”后仍然存在：
  - 视为真实问题。
  - 这说明问题属于当前代码、当前 schema、当前初始化流程或当前运行时行为。
  - 这类问题应优先修复。
- 如果问题只会出现在旧库升级、历史脏数据、旧版本错误写入或人工构造的中间态里，而彻底删库后会消失：
  - 默认视为现阶段不存在。
  - 除非它已经明显阻碍当前开发、联调或日常验证，否则不作为高优先级修复项。
- 如果删库可以解决问题，但 fresh DB 的初始化本身仍会失败：
  - 依然视为真实问题。
  - 因为这会影响新环境、新机器或全新开发数据集的正常使用。

基于这个准则，评审、测试和修复建议应遵循以下偏向：

- 优先保证：
  - fresh DB 初始化可用
  - 当前 schema 下的运行时写入路径可用
  - 当前业务语义下的核心分配/导入/导出行为可用
- 不默认追求：
  - 对所有历史版本数据库的完美迁移兼容
  - 对历史错误数据的全面自动修复
  - 仅在脏升级路径中才会触发的问题的高优先级治理

如果某个问题被判断为“删库后消失”，在 review、计划或 issue 描述里应明确标注这一点，避免把历史包袱误写成当前主问题。
