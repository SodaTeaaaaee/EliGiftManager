...
func scenarioARenderFactoryReturn(t *testing.T, lines []persistence.SupplierOrderLine) string {
	t.Helper()
	raw, err := os.ReadFile(filepath.Join("testdata", "scenario_a", "factory_return.csv.tmpl"))
	if err != nil {
...
func TestScenarioA_MonthlyMemberWave(t *testing.T) {
	...
	if !t.Run("01 import patreon members", func(t *testing.T) {
		...
		preview, err := demandController.ParseCSVFile(filepath.Join("testdata", "scenario_a", "patreon_members.csv"))
		if err != nil {
