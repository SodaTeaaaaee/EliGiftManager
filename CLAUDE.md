...
- **Desktop shell**: Wails asset pipeline, native window lifecycle via `main.go` / `app.go`
- **Portable by design**: Database and runtime data live in `data/` alongside the executable. During `wails dev`, this falls back to `<project>/data/`.
...
| `app.go` | Lifecycle hooks & bound methods |
| `internal/config/` | App metadata, window sizing |
| `internal/db/` | SQLite init, auto-migration |
| `internal/model/` | DB tables, enums, payload types |
| `internal/service/` | CSV transformers, validation, imports |
...
1. **Wails bridge boundary**: Pages and composables MUST call through `frontend/src/shared/lib/wails/app.ts`. Never import from `wailsjs` directly outside that layer.
2. **Deno-only frontend**: Use `deno task` for all frontend commands. `npm`/`yarn`/`pnpm` are not used in this project.
3. **Temporary UI shell**: The current frontend is a prototype. Business logic is not finalized. Keep UI close to Naive UI stock patterns — avoid custom design layers that will be thrown away.
