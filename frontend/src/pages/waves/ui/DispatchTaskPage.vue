...
import { allocateByTags, assignProductTag, bindDefaultAddresses, createWave, exportOrderCSV, importDispatchWave, importToWave, isWailsRuntimeAvailable, listDispatchRecords, listProductsWithTags, listTemplates, listWaves, pickCSVFile, pickZIPFile, previewExport, removeProductTag, WAILS_PREVIEW_MESSAGE, type DispatchRecordItem, type TemplateItem, type WaveItem } from '@/shared/lib/wails/app'
...
async function handleImportDispatch() {
  ...
  try { await importDispatchWave(selectedWaveId.value, csvPath.value, importTemplateId.value); message.success('发货数据导入完成'); await loadAll() }
  catch (error) { message.error(String(error)) }
}
...
<template>
  <section class="space-y-5">
    <header class="flex flex-col gap-3 xl:flex-row xl:items-end xl:justify-between">
      <div><p class="app-kicker">Dispatch Task</p><h1 class="app-title mt-2">发货任务管理</h1><p class="app-copy mt-2">创建发货任务，导入外部名单，按 Tag 匹配自动分配并导出发货清单。</p></div>
      <div class="flex gap-2">
        <NButton @click="showWaveDrawer = true">发货任务列表</NButton>
        <NButton type="primary" @click="showCreateTaskModal = true"><template #icon><NIcon><AddOutline /></NIcon></template>新建发货任务</NButton>
        <NButton :loading="isLoading" secondary strong @click="loadAll"><template #icon><NIcon><RefreshOutline /></NIcon></template>刷新</NButton>
      </div>
    </header>
    ...
    <!-- Wave Drawer -->
    <NDrawer v-model:show="showWaveDrawer" :width="420">
      <NDrawerContent title="发货任务列表" closable>
        ...
        <NDataTable :columns="waveColumns" :data="filteredWaves" :loading="isLoading" :bordered="false" :row-key="(row: WaveItem) => row.id" :row-props="(row: WaveItem) => ({ class: row.id === selectedWaveId ? 'cursor-pointer bg-slate-50 dark:bg-slate-800/60' : 'cursor-pointer', onClick: () => handleSelectWave(row) })" />
      </NDrawerContent>
    </NDrawer>
    ...
    <div v-if="selectedWave" class="space-y-4">
      <div class="flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-800/60 rounded-lg">
        ...
        <NTag :type="statusTagType(selectedWave.status)" size="small" round>{{ statusLabel(selectedWave.status) }}</NTag>
      </div>
      ...
      <!-- Step 1: Import -->
      <NCard size="small">
        <template #header><span class="flex items-center gap-2"><NIcon><CloudUploadOutline /></NIcon>步骤一：导入数据</span></template>
        <div class="grid gap-4 md:grid-cols-2">
          <div class="p-3 border border-gray-100 dark:border-gray-700 rounded-lg">
            ...
            <NFlex :wrap="false" class="mb-2"><NButton size="small" secondary @click="handlePickProductFile">选择 {{ productFileExt }}</NButton><span class="text-xs text-gray-400 self-center truncate max-w-[200px]">{{ productCsvPath || '未选择文件' }}</span></NFlex>
          ...
          <div class="p-3 border border-gray-100 dark:border-gray-700 rounded-lg">
            ...
            <NFlex :wrap="false" class="mb-2"><NButton size="small" secondary @click="handlePickCSV">选择 CSV</NButton><span class="text-xs text-gray-400 self-center truncate max-w-[200px]">{{ csvPath || '未选择文件' }}</span></NFlex>
      ...
      <!-- Step 2: Tag Management -->
      <NCard size="small">
        <template #header><span class="flex items-center gap-2"><NIcon><PlayOutline /></NIcon>步骤二：Tag 管理与分配</span></template>
        <div class="space-y-3">
          <div v-if="waveLevelTags.length > 0">
            <span class="text-xs text-gray-500 block mb-2">可选 Tag：</span>
            <NFlex :size="'small'" :wrap="true">
              <NTag v-for="tag in waveLevelTags" :key="`${tag.platform}|${tag.tagName}`" size="small" round :color="platformTagColor(tag.platform)">{{ tag.platform }}·{{ tag.tagName }}</NTag>
            </NFlex>
          </div>
          ...
          <div class="flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
            ...
            <NSelect v-model:value="selectedBatchTag" :options="batchTagOptions" placeholder="勾选 tag" size="small" style="width: 180px" clearable />
            <NButton size="small" type="primary" @click="handleAssignTag" :disabled="!selectedBatchTag || checkedProductIds.length === 0">打标</NButton>
            <NButton size="small" type="warning" @click="handleBatchRemoveTag" :disabled="!selectedBatchTag || checkedProductIds.length === 0">取消打标</NButton>
          </div>

          <NDataTable :columns="productColumns" :data="tagProducts" :loading="isTagLoading" :bordered="false" :row-key="(row: any) => row.id" v-model:checked-row-keys="checkedProductIds" :pagination="{ pageSize: 50 }" />
      ...
      <!-- Step 3: Export Preview & Edit -->
      <NCard size="small">
        <template #header><span class="flex items-center gap-2"><NIcon><DownloadOutline /></NIcon>步骤三：导出预览与编辑</span></template>
        <div class="space-y-3">
      ...
      <NCard size="small">
        <template #header><span class="flex items-center gap-2"><span>步骤四：异常检查与导出</span><NTag v-if="pendingAddressCount > 0" type="warning" size="small" round>{{ pendingAddressCount }} 条待补全</NTag></span></template>
        <template #header-extra><NButton v-if="selectedWave && pendingAddressCount > 0" size="small" type="warning" :loading="isBindingAddresses" @click="handleBindAddresses">一键补全默认地址</NButton></template>
