...
import { NAlert, NButton, NCard, NCheckbox, NDataTable, NEmpty, NForm, NFormItem, NIcon, NInput, NInputNumber, NModal, NRadio, NRadioGroup, NSelect, NSwitch, NTag, useMessage, type DataTableColumns } from 'naive-ui'
...
const form = reactive({ platform: '', type: 'import_dispatch_record', name: '', mappingRules: '{\n  \n}' })
const useGuiEditor = ref(true)
	const showCloseConfirm = ref(false)
	let formSnapshot = ''

	function snapshotForm() { formSnapshot = JSON.stringify({ platform: form.platform, type: form.type, name: form.name, mappingRules: form.mappingRules }) }
	function hasFormChanged() {
	  if (createMode.value === 'preset') return false
	  if (useGuiEditor.value) buildJsonFromGui()
	  return JSON.stringify({ platform: form.platform, type: form.type, name: form.name, mappingRules: form.mappingRules }) !== formSnapshot
	}
	function requestClose() {
	  if (!hasFormChanged()) { showCreateModal.value = false; return }
	  showCloseConfirm.value = true
	}
	function discardAndClose() { showCloseConfirm.value = false; showCreateModal.value = false }
	async function saveAndClose() { showCloseConfirm.value = false; await handleSaveTemplate() }
...
function syncGuiFromJson() {
  try { JSON.parse(form.mappingRules) } catch { return }
  ...
  if (form.type === 'import_dispatch_record') {
    ...
    guiMemberFields.value = Object.entries(mapping as Record<string, any>).map(([name, v]) => ({
      name, col: v?.columnIndex ?? 0, required: !!v?.required,
    }))
    ...
    const mapping = rules.mapping || {}
    guiProductFields.value = Object.entries(mapping as Record<string, string>).map(([name, header]) => ({
      name, header: String(header),
    }))
...
function buildJsonFromGui() {
  if (form.type === 'import_dispatch_record') {
    ...
    form.mappingRules = JSON.stringify({ prefix: guiExportPrefix.value, headers: guiExportHeaders.value }, null, 2)
  }
}
...
const columns: DataTableColumns<TemplateItem> = [
  ...
  { title: '类型', key: 'type', width: 170, render: (row) => h(NTag, { type: 'default', size: 'small', round: true }, { default: () => typeLabel(row.type) }) },
...
function openCreateModal(platform?: string) {
  editingTemplate.value = null; createMode.value = 'preset'; presetIndex.value = null
  form.platform = platform || ''; form.type = 'import_dispatch_record'; form.name = ''; form.mappingRules = '{\n  \n}'
  useGuiEditor.value = true; showCreateModal.value = true; setTimeout(snapshotForm, 0)
}
function openEditModal(template: TemplateItem) {
  editingTemplate.value = template
  form.platform = template.platform; form.type = template.type; form.name = template.name; form.mappingRules = template.mappingRules
  useGuiEditor.value = true; syncGuiFromJson()
  createMode.value = 'custom'; showCreateModal.value = true; setTimeout(snapshotForm, 0)
}
...
<template>
  <section class="space-y-5">
    <header class="flex flex-col gap-3 xl:flex-row xl:items-end xl:justify-between">
      ...
      <NButton type="primary" @click="openCreateModal()"><template #icon><NIcon><AddOutline /></NIcon></template>新建模板</NButton>
    </header>
    ...
    <div v-else-if="platforms.filter(p => p !== 'all').length" class="grid gap-4 xl:grid-cols-2">
      <NCard v-for="platform in platforms.filter(p => p !== 'all')" :key="platform" size="small">
        ...
        <NAlert v-if="hasImportProduct(platform) && !hasExportOrder(platform)" type="warning" :show-icon="false" class="mb-3">
          此平台有商品导入模板但缺少订单导出模板，建议补全。
        </NAlert>
        <NDataTable v-if="platformTemplates(platform).length" :columns="columns" :data="platformTemplates(platform)" :bordered="false" :pagination="false" size="small" />
    ...
    <!-- ====== 模板编辑弹窗 ====== -->
    <NModal v-model:show="showCreateModal" preset="card" :title="modalTitle" :mask-closable="false" style="max-width: 860px; width: 90vw">
      <NForm label-placement="top">
        ...
        <template v-if="createMode === 'preset'">
          ...
          <NFormItem v-if="selectedPreset" label="模板预览">
            <NCard size="small"><pre class="text-xs whitespace-pre-wrap">{{ JSON.stringify(JSON.parse(selectedPreset.mappingRules), null, 2) }}</pre></NCard>
          </NFormItem>
        </template>
        ...
        <template v-else>
          ...
          <NFormItem label="模板类型">
            <NSelect v-model:value="form.type" :options="typeOptions" @update:value="onTypeChange" />
          </NFormItem>
          ...
          <div class="mb-4">
            <div class="flex items-center justify-between mb-2">
              ...
              <div class="flex items-center gap-2">
                <span class="text-xs" :class="useGuiEditor ? 'text-[var(--primary)]' : 'text-gray-400'">GUI</span>
                <NSwitch v-model:value="useGuiEditor" size="small" />
                <span class="text-xs" :class="!useGuiEditor ? 'text-[var(--primary)]' : 'text-gray-400'">Raw</span>
              </div>
            </div>

            <!-- GUI 编辑器 -->
            <div v-if="useGuiEditor" class="border rounded-lg p-3 space-y-3 bg-gray-50/50 dark:bg-gray-800/30">

              <!-- 会员导入 / 发货记录导入 -->
              <template v-if="form.type === 'import_dispatch_record'">
                <NCheckbox :checked="guiHasHeader" @update:checked="v => guiHasHeader = v" size="small">CSV 包含表头行</NCheckbox>
                <div>
                  <div class="text-xs text-gray-500 mb-1">字段映射</div>
                  <div class="space-y-1">
                    <div v-for="(f, i) in guiMemberFields" :key="i" class="flex items-center gap-1">
                      <NSelect v-model:value="f.name" :options="memberFieldOptions" size="small" style="width:130px" />
                      <span class="text-xs text-gray-400 shrink-0">第</span>
                      <NInputNumber v-model:value="f.col" size="small" :min="0" style="width:64px" />
                      <span class="text-xs text-gray-400 shrink-0">列</span>
                      <NCheckbox :checked="f.required" @update:checked="v => f.required = v" size="small" class="shrink-0">必填</NCheckbox>
                      <NButton size="tiny" quaternary type="error" @click="guiMemberFields.splice(i, 1)">✕</NButton>
                    </div>
                  </div>
                  <NButton size="tiny" quaternary class="mt-1" @click="guiMemberFields.push({ name: '', col: guiMemberFields.length, required: false })">+ 添加字段</NButton>
                </div>
              </template>
              ...
              <template v-else-if="form.type === 'import_product'">
                ...
                <div>
                  <div class="text-xs text-gray-500 mb-1">字段映射</div>
                  <div class="space-y-1">
                    <div v-for="(f, i) in guiProductFields" :key="i" class="flex items-center gap-1">
                      <NSelect v-model:value="f.name" :options="productFieldOptions" size="small" style="width:130px" />
                      <span class="text-xs text-gray-400">→</span>
                      <NInput v-model:value="f.header" size="small" class="flex-1" placeholder="CSV 表头名" />
                      <NButton size="tiny" quaternary type="error" @click="guiProductFields.splice(i, 1)">✕</NButton>
                    </div>
                  </div>
                  <NButton size="tiny" quaternary class="mt-1" @click="guiProductFields.push({ name: '', header: '' })">+ 添加字段</NButton>
                </div>
              </template>
              ...
              <template v-else-if="form.type === 'export_order'">
                ...
                <div>
                  ...
                  <div class="space-y-1">
                    <div v-for="(h, i) in guiExportHeaders" :key="i" class="flex items-center gap-1">
                      <span class="text-xs text-gray-400 w-4 shrink-0">{{ i + 1 }}</span>
                      <NInput v-model:value="guiExportHeaders[i]" size="small" class="flex-1" />
                      <NButton size="tiny" quaternary type="error" @click="guiExportHeaders.splice(i, 1)">✕</NButton>
                    </div>
                  </div>
                  <NButton size="tiny" quaternary class="mt-1" @click="guiExportHeaders.push('')">+ 添加表头</NButton>
                </div>
              </template>

              <!-- allocation / 其他 → 降级提示 -->
              <template v-else>
                <p class="text-xs text-gray-400">此模板类型暂无 GUI 编辑器，请切换到 Raw 模式编辑。</p>
              </template>
            </div>

            <!-- Raw JSON -->
            <NInput v-else v-model:value="form.mappingRules" type="textarea" :autosize="{ minRows: 6 }" placeholder='例如：{"platform_uid":"用户ID"}' />
          </div>
        </template>

        <div class="flex gap-2 mt-2">
          <NButton secondary @click="requestClose">关闭</NButton>
          <NButton type="primary" :loading="isSaving" class="flex-1" @click="handleSaveTemplate">{{ editingTemplate ? '保存修改' : '创建模板' }}</NButton>
        </div>
      </NForm>
    </NModal>

    <!-- 未保存确认 -->
    <NModal v-model:show="showCloseConfirm" preset="card" title="未保存的更改" :mask-closable="false" style="max-width: 400px">
