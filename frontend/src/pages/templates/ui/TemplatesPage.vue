...
import { NButton, NCard, NDataTable, NEmpty, NForm, NFormItem, NIcon, NInput, NModal, NSelect, NTabPane, NTabs, NTag, useMessage, type DataTableColumns } from 'naive-ui'
import { createTemplate, isWailsRuntimeAvailable, listTemplates, WAILS_PREVIEW_MESSAGE, type TemplateItem } from '@/shared/lib/wails/app'
...
function openCreateModal() { form.platform = activePlatform.value !== 'all' && activePlatform.value !== '通用' ? activePlatform.value : ''; form.type = 'import_member'; form.name = ''; form.mappingRules = '{\n  \n}'; showCreateModal.value = true }
...
async function handleCreateTemplate() {
  isSaving.value = true
  try { const template = await createTemplate(form.platform, form.type, form.name, form.mappingRules); message.success(`已创建模板：${template.name}`); showCreateModal.value = false; await loadTemplates(); activePlatform.value = template.platform || 'all' } catch (error) { message.error(String(error)) } finally { isSaving.value = false }
}
