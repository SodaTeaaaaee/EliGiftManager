...
export const zhCN = {
  ...
  inbox: {
    ...
    issues: '解析提示',
    noTemplateHint: '该平台还没有可用的活跃输入模板，先到资料库复制内置模板或新建一个。',
    goToLibraryTemplates: '去资料库添加模板',
    importPreview: '导入预览',
    importPreviewHint: '下方是按所选模板解析文件的结果，确认无误后再导入。更换模板或文件会重新预览。',
    confirmImport: '确认导入',
    importFileSuccess: '文件已导入收件箱',
  ...
  waveWorkspace: {
    ...
    noShipmentTemplate: '该工厂平台暂无发货回传模板',
    noShipmentTemplateHint: '发货回传需要先为该工厂平台添加一个活跃的「发货回传」模板。',
   ...
   * lists, the editor drawer (归属 / 样例文件 / 映射 / 布局 / 验证), the template
   * test drawer, and the carrier-mapping section.
   */
  templates: {
    test: '测试',
    testTitle: '模板测试',
    testIdle: '选择样例文件后自动运行解析。内置模板也可以在这里测试。',
    rerunTest: '重新解析',
    uploadSample: '选择样例文件',
    changeSample: '更换样例文件',
    loadFailed: '加载模板数据失败',
    ...
    list: {
      activeTitle: '活跃模板',
      activeDesc: '导入、导出和回写实际使用的模板。可编辑、测试、删除。',
      emptyActiveTitle: '还没有活跃模板',
      emptyActiveDesc: '从下方的内置模板目录「复制为活跃模板」最快，也可以从空白新建。',
      builtinTitle: '内置模板',
      builtinDesc: '只读目录，不能直接用于导入或导出。复制成活跃模板后再按需调整。',
      emptyBuiltin: '暂无内置模板',
      builtinTag: '内置',
      inEffect: '当前生效',
      configSummary: '配置摘要',
      updatedAt: '更新时间',
      unknownPlatform: '平台 #{id}',
      copyToActive: '复制为活跃模板',
      deleteAction: '删除',
      deleteConfirm: '确定删除模板「{name}」？历史导入、导出记录仍保留其版本快照。',
    },
    editor: {
      createTitle: '新建模板',
      editTitle: '编辑模板：{name}',
      copyTitle: '复制内置模板：{name}',
      copyNameSuffix: '{name}（副本）',
      sectionOwnership: '归属',
      sectionOwnershipHint: '单据类型决定方向和可选平台；三者在创建后不可更改。',
      catalogUnavailable: '无法读取单据类型目录，请刷新页面后重试。',
      locked: '创建后不可更改',
      directionLocked: '由单据类型决定',
      platform: '所属平台',
      platformKindHint: '仅显示{kind}',
      noPlatformForKind: '没有{kind}，请先在设置中登记',
      namePlaceholder: '例如：B 站会员名单（月度）',
      notesPlaceholder: '记录这份模板对应的文件来源、导出路径或注意事项',
      sectionSample: '样例文件',
      sectionSampleHint: '读取真实或示例文件的前几行，用来选择来源列并即时预览。样例文件不会被保存。',
      sectionMapping: '字段映射',
      sectionMappingHint: '把来源列对齐到内部语义字典。未映射的列不会被导入。',
      sectionValidate: '验证',
      sectionValidateHint:
        '建议在保存前用样例文件跑一次完整解析（含枚举映射、日期、行拆分和必填丢弃），但不是必需的。',
      runPreview: '用样例文件预览',
      previewNeedsSample: '先在「样例文件」中选择文件',
      previewIdle: '点击「用样例文件预览」查看完整解析结果、丢弃行数和重复指纹。',
      sectionLayout: '输出布局',
      sectionLayoutHint: '选择并排序输出列，设置平台需要的表头。至少需要一列。',
      saveCreate: '创建模板',
      saveUpdate: '保存修改',
      saveFailed: '保存模板失败',
    },
    sample: {
      ...
      headerModeHint: '按表头名称绑定来源列',
      positionalModeHint: '按 0 起始的列号绑定来源列',
      inspectFailed: '读取样例文件失败：{message}',
      format: '格式 {format}',
      rowSummary: '显示 {shown} 行 / 共 {total} 行',
      emptyFile: '文件里没有可读取的行',
    },
    layoutEditor: {
      available: '可用字段',
      availableHint: '点击字段加入输出列。',
      allSelected: '所有字段都已加入输出列',
      otherGroup: '其他',
      selected: '输出列（{n}）',
      selectedHint: '按从左到右的顺序输出；显示表头留空时使用字段名称。',
      noColumns: '还没有输出列，从左侧选择字段',
      headerPlaceholder: '显示表头（默认：{label}）',
      moveUp: '上移',
      moveDown: '下移',
      headerPreview: '表头预览',
      headerPreviewHint: '第一行是导出文件的表头，第二行是占位示例数据。',
    },
    carrier: {
      desc: '来源平台回填时接受的承运商 ID 对照：左侧是工厂发货文件中出现的承运商名称或描述，右侧是该平台接受的承运商 ID。',
      nameHeader: '名称或描述',
      codeHeader: '平台承运商 ID',
      add: '添加',
      addNamePlaceholder: '名称或描述，例如：申通快递',
      addCodePlaceholder: '平台承运商 ID，例如：shentong',
      empty: '该平台暂无承运商映射',
      noSourcePlatform: '暂无来源平台',
      updateSuccess: '承运商映射已更新',
      deleteSuccess: '承运商映射已删除',
      deleteConfirm: '确定删除「{name}」的映射？',
      importFromFile: '从文件导入',
      importTitle: '从文件导入承运商映射',
      importPickHint: '选择一份包含承运商名称和平台承运商 ID 两列的 CSV / Excel 文件，然后指定这两列。',
      importPickFile: '选择文件',
      importNameColumn: '名称或描述所在列',
      importCodeColumn: '平台承运商 ID 所在列',
      importSameColumn: '名称列和 ID 列不能相同',
      importNoHeaders: '没有读到表头行，请确认文件第一行是表头',
  ...
  templateEditor: {
    ...
    previewTitle: '本地即时预览',
    previewNote: '本地预览仅提示 trim / 去引号 / 合并列；完整解析（枚举映射、日期、行拆分、必填丢弃）请在「验证」中用样例文件预览。',
    showAllFields: '显示全部字段',
    showAllFieldsWithCount: '显示全部字段（另有 {n} 个）',
    groupBoundCount: '{bound} / {total} 已映射',
    groups: {
      identity: '客户与身份',
      source: '来源',
      product: '商品与数量',
