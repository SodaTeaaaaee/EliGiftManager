...
export function useAdaptiveTable<T>(items: Ref<T[]>, opts: AdaptiveTableOptions) {
  ...
  async function remeasure() {
    ...
    if (trs && trs.length > 0) {
      measuredHeights.value = Array.from(trs).map((tr) => (tr as HTMLElement).offsetHeight)
    }
  ...
  function setupResizeObserver() {
    ...
    if (opts.tableParentRef.value) resizeObserver.observe(opts.tableParentRef.value)
  }
