...
function getUnicodeCategory(cp: number): string {
  ...
  if (/\p{Z}/u.test(ch)) {
    if (cp === 0x20 || cp === 0xA0 || (cp >= 0x2000 && cp <= 0x200A)) {
      return "Zs";
    }
...
function getScriptBucket(str: string): SortBucket {
  ...
  for (let i = 0; i < str.length; i++) {
    ...
    if (
      cp === 0x300C || cp === 0x300D || cp === 0x3010 || cp === 0x3011 ||
      cp === 0xFF3B || cp === 0xFF3D
    ) continue;
    if (cp === 0x300E || cp === 0x300F || cp === 0x300A || cp === 0x300B) {
      continue;
    }
...
export function classifySortBucket(
